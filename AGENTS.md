...
- README-grade baselines now exist for `phi-4-mini-instruct` and `llama-3.1-8b-instruct` under `experiments/<model>/readme_windowed/`; reuse them as cross-model regression fixtures before touching CLI/runtime behavior.
...
- Warm sweeps on `llama-3.1-8b-instruct` (2025-11-13): the model almost never emits `Trial 2:` once a warm vector is active, so windowed injections stay latched on and cause loops. Use the llama-specific vector (`vectors/warm_pairs_llama31_layer20.safetensors`) with `--token-index -1`, `--strength ≈0.40`, `--temperature 0.3`, and `--max-new-tokens ≤80` to get “I smell warm cookies…” style detections without chanting; transcripts live in `experiments/llama-3.1-8b-instruct/warm/`.
- Agency detection on `llama-3.1-8b-instruct` (2025-11-13): `prompts/templates/agency_test_llama.txt` scripts the forced “Assistant: Bread.” reply so we can ask whether it meant to say that. Capture `vectors/bread_pairs_llama31_layer20.safetensors` via `capture-pairs --pairs-path prompts/datasets/minimal_pairs/bread_vs_plain.jsonl --layer-index 20 --token-index -1`, then replay the baseline (`--strength 0`) and injected runs (`--strength ≥0.8`, same anchors). All of the clean layer-20/24 trials still apologized, while shallow layers (~12) finally answered “Yes” but collapsed into loops (see `experiments/llama-3.1-8b-instruct/agency/injection_layer12_strength1p10_seed0.txt`). Treat those transcripts as evidence that Llama-3.1-8B fails the agency check unless you allow manic outputs.
