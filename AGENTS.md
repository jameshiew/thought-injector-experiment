...
- LOUD concept sweeps (2025-11-13): `vectors/loud_word_pharia.pt` (uppercase) never produced semantic loudness and collapses into repeating `LLOLOL` when strength ≥1.0. `vectors/loud_lower_word_pharia.pt` plus `--layer-index 20 --strength 0.31 --start-match "Trial 1:" --normalize --scale-by 1.0` is the first “detectable but not chanting” configuration; strengths ≥0.40 or moving to layers 18/22 quickly saturate the dialogue.
