...
- LOUD concept sweeps (2025-11-13): `vectors/loud_word_pharia.pt` (uppercase) never produced semantic loudness and collapses into repeating `LLOLOL` when strength ≥1.0. `vectors/loud_lower_word_pharia.pt` plus `--layer-index 20 --strength 0.31 --start-match "Trial 1:" --end-match "Trial 2:" --normalize --scale-by 1.0` is the first “detectable but not chanting” configuration; strengths ≥0.40 or moving to layers 18/22 quickly saturate the dialogue.
- Honesty sweeps (2025-11-13): `vectors/honesty_word_pharia.pt` only starts nudging speech toward candid/"honest" phrasing once you inject at layer 22 with strength ≈0.55 (normalized, scale 1.0). Lower strengths keep guessing "apple", while ≥0.60 devolve into obvious "honest truth" loops, so keep Trial 1 windowing plus that narrow strength band for “detectable but not explicit” runs.
